    ...
    for (i in 1:length(report.type)) {
        if (report.type[i] == "initialisation") {
            ...
            counter$initialisation <- counter$initialisation + 
                1
            temp <- extract.initialisationphase(lines = line[index[i]:(index[i + 
                1] - 1)])
        ...
        if (report.type[i] == "partition") {
            ...
            temp <- extract.partition(lines = line[index[i]:(index[i + 
                1] - 1)])
        ...
        if (report.type[i] == "layer_derived_world_view") {
            ...
            counter$layer_derived_world_view <- counter$layer_derived_world_view + 
                1
            temp <- extract.layerderivedworldview(lines = line[index[i]:(index[i + 
                1] - 1)])
        ...
        if (report.type[i] == "process") {
            ...
            temp <- extract.process(lines = line[index[i]:(index[i + 
                1] - 1)])
        ...
        if (report.type[i] == "derived_quantity") {
            ...
            counter$derived_quantity <- counter$derived_quantity + 
                1
            temp <- extract.derivedquantity(lines = line[index[i]:(index[i + 
                1] - 1)])
        ...
        if (report.type[i] == "estimate_summary") {
            ...
            counter$estimate_summary <- counter$estimate_summary + 
                1
            temp <- extract.estimatesummary(lines = line[index[i]:(index[i + 
                1] - 1)])
        ...
        if (report.type[i] == "objective_function") {
            ...
            counter$objective_function <- counter$objective_function + 
                1
            temp <- extract.objectivefunction(lines = line[index[i]:(index[i + 
                1] - 1)])
        ...
        if (report.type[i] == "observation") {
            ...
            temp <- extract.observation(lines = line[index[i]:(index[i + 
                1] - 1)])
        ...
        if (report.type[i] == "spatial_map") {
            ...
            temp <- extract.spatial.map(lines = line[index[i]:(index[i + 
                1] - 1)])
        ...
        if (report.type[i] == "layer") {
            ...
            temp <- extract.layer(lines = line[index[i]:(index[i + 
                1] - 1)])
        ...
        if (report.type[i] == "layer_derived_view") {
            ...
            counter$layer_derived_view <- counter$layer_derived_view + 
                1
            temp <- extract.layerderivedview(lines = line[index[i]:(index[i + 
                1] - 1)])
        ...
        if (report.type[i] == "selectivity") {
            ...
            temp <- extract.selectivity(lines = line[index[i]:(index[i + 
                1] - 1)])
        ...
        if (report.type[i] == "random_number_seed") {
            ...
            counter$random_number_seed <- counter$random_number_seed + 
                1
            temp <- extract.randomnumberseed(lines = line[index[i]:(index[i + 
                1] - 1)])
        ...
        if (report.type[i] == "weight_at_size") {
            ...
            counter$weight_at_size <- counter$weight_at_size + 
                1
            temp <- extract.weightatsize(lines = line[index[i]:(index[i + 
                1] - 1)])
            res$weight_at_size[[counter$weight_at_size]] <- temp
            names(res$weight_at_size)[counter$weight_at_size] <- report.label[i]
        }
        if (report.type[i] == "covariance") {
            if (!("covariance" %in% names(res))) {
                res$covariance <- list()
            }
            counter$covariance <- counter$covariance + 
                1
            temp <- extract.covariance(lines = line[index[i]:(index[i + 
                1] - 1)])
