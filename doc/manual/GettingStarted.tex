...
\SPM\ has a range of commands that define the model structure, processes, observations, how tasks are carried out. Some commands can be used multiple times in the same parameter file (for example, \command{population\_process}) and for these commands, most have a label (that must be unique). Some commands can be used multiple times, but do not have a label (for example, \command{estimate}), and the remainder can be used only once and may never have a label (such as \command{spatial\_structure}). 
