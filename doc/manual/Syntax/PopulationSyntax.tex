...
\begin{itemize}
  ...
  \item Local Beverton-Holt stock-recruit relationship recruitment process
	...
	\item Holling mortality process
	\item Prey switch predation mortality process
...
\defSub{b0} {Define the \command{initialization\_phase} label for the value of the derived quantity  to use as the value of the spawning stock biomass ($B_0$)}
\defType{String}
\defDefault{No default}
\defValue{Must be a valid \command{initialization\_phase} label}
...
\defSub{b0} {Define the \command{initialization\_phase} label for the value of the derived quantity  to use as the value of the spawning stock biomass ($B_0$) in each cell}
\defType{String}
\defDefault{No default}
\defValue{Must be a valid \command{initialization\_phase} label}
...
\TODO
\subsubsection[Predation mortality process]{\commandlabsubarg{process}{type}{predation\_mortality\_rate}}

\defSub{type}{Define the type of predation mortality that is applied}
\defType{String}
\defDefault{Holling}
\defValue{Must be either Switch or Holling}

\defSub{as\_biomass}{Is the mortality applied as a biomass or as abundance}
\defType{Switch}
\defDefault{True}
\defValue{Either True or False}

\defSub{preys}{Define the labels of the prey groups}
\defType{String vector}
\defDefault{No default, only needed if \subcommand{type} is Switch}
\defValue{Names of prey must be unique}

\defSub{electivities}{Define the electivities applied to each prey group}
\defType{Constant vector, of length \subcommand{preys}}
\defDefault{No default, only needed if \subcommand{type} is Switch}
\defValue{A vector of positive real numbers, must sum to 1}

\defSub{a}{Define the $a$ parameter of the Holling function}
\defType{Constant}
\defDefault{No default, only needed if \subcommand{type} is Holling}
\defValue{A positive real number}
...
\defType{Constant}
\defDefault{default $2$, only needed if \subcommand{type} is Holling}
\defValue{A positive real number, use $2$ for Holling type II or $3$ for Holling Type III}

\defSub{categories}{Define the categories that the predation mortality rate is applied to}
\defType{String vector}
\defDefault{No default}
\defValue{Valid categories from \commandsub{model}{categories}}
...
\defSub{prey\_groups}{Define which prey group each category belongs to}
\defType{String vector, of length \subcommand{categories}}
\defDefault{No default, only needed if \subcommand{type} is Switch}
\defValue{Valid labels defined by \command{preys}}

\defSub{categories\_predator}{Define the categories of the predator}
\defType{String vector}
\defDefault{No default}
\defValue{Valid categories from \commandsub{model}{categories}}

\defSub{selectivities\_predator}{Define the selectivities applied to each predator category}
\defType{String vector, of length \subcommand{categories\_predator}}
\defDefault{No default}
\defValue{Valid selectivity labels defined by \command{selectivity}}
...
\defSub{x}{Define the $x$ parameter of the Holling function. Use $x=2$ for Type II or $x=3$ for Type III}
\defType{Numeric}
\defDefault{2}
\defValue{A positive real number $\geq 1.0$}
...
\subsubsection[Prey switching predation mortality process]{\commandlabsubarg{process}{type}{switch\_mortality\_rate}}
...
\defSub{prey}{Define arbitrary labels for all the prey groups used}
\defType{String vector}
\defDefault{No default}
\defValue{Names must be unique}

\defSub{electivities}{Define the electivities applied to prey groups 1 to n-1}
\defType{Constant vector, of length $\subcommand{prey} - 1$}
\defDefault{No default}
\defValue{A vector of positive real numbers, must sum to $< 1$}

\defSub{categories}{Define the categories that the predation mortality rate is applied to for all prey groups}
\defType{String vector}
\defDefault{No default}
\defValue{Valid categories from \commandsub{model}{categories}}

\defSub{selectivities}{Define the selectivities applied to each prey category}
\defType{String vector, of length \subcommand{categories}}
\defDefault{No default}
\defValue{Valid selectivity labels defined by \command{selectivity}}
...
%\defValue{Must be valid model years once expanded, including initialisation steps}
%
%\defSub{time\_step} {Define the time-step at which the calculation is performed}
%\defType{String}
%\defDefault{No default}
...
\subsection{Age-size relationship}
