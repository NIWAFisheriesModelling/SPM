...
\defSub{SizeWeight}{Define the size-weight relationships for each of the categories that the event mortality is applied to}
\defType{String vector}
\defDefault{None}
\defValue{Valid labels from \command{SizeWeight}}
...
\defSub{SizeWeight}{Define the size-weight relationships for each of the categories that the biomass is calculated from}
\defType{String vector}
\defDefault{None}
\defValue{Valid labels from \command{SizeWeight}}
...
The \subsubsection[Distance]{\commandsubarg{Layer}{Type}{Distance}} layer has no other subcommands.
