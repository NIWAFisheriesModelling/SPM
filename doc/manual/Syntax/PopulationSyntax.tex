...
\defSub{prey}{Define the vector of labels for all the prey groups}
\defType{String vector}
\defDefault{No default}
...
\defSub{a}{Define the $a$ vector of the Holling function for electivity for all the prey groups}
\defType{Constant vector, of length $\subcommand{prey}$}
\defDefault{No default}
\defValue{A vector of positive real numbers}

\defSub{b}{Define the $b$ vector of the Holling function for electivity for all the prey groups}
\defType{Constant vector, of length $\subcommand{prey}$}
\defDefault{Default $0$}
\defValue{A vector of positive real numbers}

\defSub{x}{Define the type of Holling function or Michaelis-Menton function for electivity}
\defType{Constant}
\defDefault{default $1$}
\defValue{A positive real number, use $1$ for constant, $2$ for Holling type II or $3$ for Holling Type III, or other positive real value for the generalised Michaelis-Menton function}

%
%\defSub{electivities}{Define the electivities applied to prey groups $1$ \ldots $n-1$}
...
\defSub{categories}{Define the categories that the predation mortality is applied to, for all prey groups}
\defType{String vector}
\defDefault{No default}
\defValue{Valid categories from \commandsub{model}{categories}}
