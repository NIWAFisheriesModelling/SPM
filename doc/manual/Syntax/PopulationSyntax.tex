...
Name changed from layer to layers, needs to be one per year.
...
\defDefault{No layer}
\defValue{A vector of valid layers as defined by \command{layer}. If a numeric layer, then mortality applied is the mortality rate times the value of the layer. Note that the layer values must be non-negative}
