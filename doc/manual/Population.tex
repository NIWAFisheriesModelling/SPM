...
Note that if not specified, $SSB\_offset$ is set at the value of $age$. This corresponds to cases where recruitment happens after spawning. $SSB\_offset$ can be user-defined to a different value if needed, such as if recruitment happens in a time-step before spawning, and therefore the offset is $age + 1$.
...
Note that the local Beverton-Holt recruitment process requires a value for $B_0^i$ and $SSB_y^i$ to resolve the stock-recruitment relationship. Here, layers (that probably should be abundance or biomass layers --- see Section \ref{sec:derived-layers}) must be provided for $B_0$ and the annual $SSB_y$ for the recruitment process. $B_0$ is then defined as the value of the $SSB$ at the end of one of the initialisation phases. During initialisation the YSC multipliers are assumed to be equal to one, and recruitment that happens in the initialisation phases that occur before and during the phase when $B0$ is determined is assumed to have steepness $h=1$ (i.e. in those initialisation phases, recruitment is simply equal to $R_0$). Recruitment in the initialisation phases after the phase where $B_0$ was determined follow the Beverton-Holt stock-recruit relationship defined above. $R_0^i$ is defined for each cell as the portion of the total recruitment $R_0$ distributed relative to the recruitment layer \texttt{R0\_layer}.

For example, assume a local Beverton-Holt recruitment process, where individuals are added to the category `immature' at $age=1$, the number to add is $R_0=5 \times 10^5$ multiplied by the proportional value of the layer \texttt{recruitment\_layer}, $B_0$ is a derived layer that specifies the biomass in each cell  of total spawning stock biomass at the end of initialisation, $SSB_y$ is a derived quantity labelled \texttt{SSB\_biomass} that represents the spawning stock biomass in each year, with $B_0$ the value of the derived quantity at the end of the initialisation phase labelled \texttt{phase1}. The YCS are standardised to have mean one in the period 1994 to 2004, and recruits enter into the model two years following spawning. Then the command specification is,
...
The Holling mortality process\index{Holling mortality} applies either a Holling Type II or Type III function that removes a number or biomass from a set of categories according to their abundance (or biomass) and some 'predator' abundance (or biomass) .

For example, the Holling mortality applied to user-defined categories $k$, with the numbers removed at age $l$ determined by a selectivity-at-age $S_l$ is applied as follows:

First, calculate the vulnerable abundance for each category $k$ in $1 \ldots K$ for ages $l = 1 \ldots L$ that are subject to the mortality,
\begin{equation}
  V(k,l) = S(l) N(k,l)
\end{equation}

and then, assuming that the predator abundance (or biomass) is given by the value of an abundance or biomass layer $L_a$ at $a$, the the number to remove is determined as,
\begin{equation}
R(k,l) = \begin{cases}
  L_a (a  V(k,l))/(b + V(k,l)), & \text{if Type II} \\
  L_a (a^2  V(k,l)^2)/(b^2 + V(k,l)^2), & \text{if Type III}\\ 
  \end{cases} 
\end{equation}

Except that if the exploitation rate ($U=R(l,k)/V(l,k)$) in any partition element $(l,k)$ exceeds $U_{max}$, then the exploration rate is set equal to $U_{max}$.

Note that the Holling function is applied either as an abundance (if $L_a$ is an abundance layer) or biomass (if $L_a$ is a biomass layer).
