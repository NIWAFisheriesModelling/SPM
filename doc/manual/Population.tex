...
First, calculate the total vulnerable abundance (or biomass) over all categories $k$ in $1 \ldots K$ and ages $l = 1 \ldots L$ that are subject to the mortality,
\begin{equation}
  V(k,l) = S(l) N(k,l)
\end{equation}
...
\begin{equation}
R_{total} = L_a (a  V_{total}^{x-1})/(b + V_{total}^{x-1})
\end{equation}

Hence the exploitation rate\index{Maximum exploitation rate} to apply is 
\begin{equation}
U = \begin{cases}
  R_{total}/V_{total}, & \text{if $C/V_{total} \leq U_{max}$} \\
  U_{max}, & \text{otherwise}\\ 
  \end{cases} 
\end{equation}
...
Note that the mortality function will be applied as an abundance if $L_a$ is an abundance layer, or biomass if $L_a$ is a biomass layer.
