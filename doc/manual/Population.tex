...
The density-dependent predation mortality process can be applied using Holling functions (Type II or Type III), or a prey-switching function. All functions remove a number or biomass from a set of categories according to their total (selected) abundance (or biomass), some 'predator' abundance (or biomass), and constrained by a maximum exploitation rate. In the case of the prey switching function, the removal of any prey species is also a function of the total (selected) abundance or biomass of all other prey species using their electivity. Note that the mortality function will be applied as a biomass or an abundance depending on the value of the \subcommand{as\_biomass} switch. 
...
For example, the mortality applied to user-defined categories $k$, with the numbers removed at age $l$ determined by a selectivity-at-age $S_l$ is applied as follows:
...
And define the total predator abundance (or biomass) $V_{total}$ as,
\begin{equation}
...
\begin{equation}
R_{total} =\displaystyle\begin{cases}
  P_{total} (a  V_{total})/(b + V_{total}),&\text{for Holling Type II}\\
  P_{total} (a  V_{total}^2)/(b + V_{total}^2),&\text{for Holling Type III}\\
\end{cases}
\end{equation}
...
\TODO
Then the number to remove for each prey group over all groups $g$ $1 \ldots G$ 
\TODOend
