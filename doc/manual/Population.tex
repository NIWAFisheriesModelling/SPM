...
In addition, each initialisation process can optionally be stopped early if a user defined convergence criteria is met. For a set of user defined years in the initialisation phase, convergence is defined as the proportional absolute summed difference between the the state in year $t-1$ and the state in year $t$, i.e., 
\begin{equation}
  \begin{aligned}
    N_{t-1} =& \sum\limits_{i} \sum\limits_{j} \sum\limits_{k} \sum\limits_l \text{element}(i,j,k,l) \textrm{ at the end of year } t-1 \textrm{, and} \\
	  N_{t} =& \sum\limits_{i} \sum\limits_{j} \sum\limits_{k} \sum\limits_l \text{element}(i,j,k,l) \textrm{ at the end of year } t, \\
	  \lambda =& (N_{t}-N_{t-1})/N_{t}
  \end{aligned}
\end{equation}
