...
\begin{itemize}
  ...
  \begin{equation}
    w=a l^b
  \end{equation}
	Note that a bias correction is applied if there is a size-at-age distribution. The mean weight $w$ is corrected using the distribution $cv$, so that
  \begin{equation}
	  w=(al^b)(1+cv^2)^{(b(b-1)/2)}
  \end{equation}
\end{itemize}
