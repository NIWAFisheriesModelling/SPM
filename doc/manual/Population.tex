...
The names and number of categories are user defined, but there must be at least one category defined for a model. The ages are defined as a sequence from $age_{min}$ to $age_{max}$, with the last age optionally a plus group. In order to calcuate bio mass, the age-size relationship for each category must also be defined.
...
<<<<<<< .mine
Six types of mortality processes are permissible in \SPM, constant rate, annually varying rate, event, biomass-event, a density-dependent relationship based on the Holling \citep{Holling1959} Type II or Type III function, and a density-dependent relationship based on prey-switching mechanisms. These processes remove individuals from the partition, either as a rate (for constant or annually varying mortality processes), as a total number (abundance), or as a biomass of individuals. \SPM\ does not implement the Baranov catch equation or any other process where both natural and event mortality are applied simultaneously. To approximate concurrent natural and event mortality, the population processes must be defined to remove some natural mortality (e.g., as a constant or annually varying), then some event mortality (e.g., fishing) in sequence. It is up to the user to specify how this happens.
=======
Five types of mortality processes are permissible in \SPM, constant rate, annually varying rate, event, biomass-event, and density-dependent predation mortality. These processes remove individuals from the partition, either as a rate (for constant or annually varying mortality processes), as a total number (abundance), or as a biomass of individuals. \SPM\ does not implement the Baranov catch equation or any other process where both natural and event mortality are applied simultaneously. To approximate concurrent natural and event mortality, the population processes must be defined to remove some natural mortality (e.g., as a constant or annually varying), then some event mortality (e.g., fishing) in sequence. It is up to the user to specify how this happens.
>>>>>>> .r4848
...
=======
The constant rate, annually varying rate, event, and biomass-event mortalities can depend on a layer. In these cases, the value of instantaneous mortality applied to the population state within each cell is the product of the layer value, a selectivity, and the mortality rate. If the layer is static, mortality is effectively constant each year but can be different in each cell; if the layer is a derived layer and therefore calculated each year, mortality can change every year in every cell. 
>>>>>>> .r4848

\TODO

Complex mortality relationships such as density dependent predation mortality with prey selectivity can be applied through user-defined derived layers (see Section \ref{sec:derived-layers} for an example). 
\TODOend
...
<<<<<<< .mine
The density-dependent Holling mortality process\index{Holling mortality} applies the Holling Type II and Type III functions \citep{Holling1959}, but is generalised using the Michaelis-Menten equation \citep{MentenMichaelis1913}. The function removes a number or biomass from a set of categories according to their total (selected) abundance (or biomass), some 'predator' abundance (or biomass), and constrained by a maximum exploitation rate.
=======
Three types of density-dependent predation mortalities are available: two relationships based on the Holling \citep{Holling1959} Type II or Type III functions which use predator and target prey density; and a prey switching relationship based on predator density, predator electivity and the density of all its preys. All functions remove a number or biomass from a set of categories according to their total (selected) abundance (or biomass), some 'predator' abundance (or biomass), and constrained by a maximum exploitation rate. In the case of the prey switching function, the removal of any prey is also a function of the total (selected) abundance or biomass of all other prey groups and the electivity of the predator to each prey group. Note that the mortality function will be applied as a biomass or an abundance depending on the value of the \subcommand{as\_biomass} switch. 
>>>>>>> .r4848
...
=======
The density-dependent Holling mortality process\index{Holling mortality} applies the Holling Type II and Type III functions \citep{Holling1959}, but generalised using the Michaelis-Menten equation \citep{MentenMichaelis1913}. 
>>>>>>> .r4848
...
Then, calculate the total vulnerable abundance (or biomass) over all categories $k$ in $1 \ldots K$ and ages $l = 1 \ldots L$ that are subject to the mortality,
\begin{equation}
=======
  P(k,l) = S(l) N(k,l)
\end{equation}
...
>>>>>>> .r4848
\begin{equation}
R_{total} = P_{total} (a  V_{total}^{x-1})/(b + V_{total}^{x-1})
\end{equation}

Chose $x=2$ for Holling type II function and $x=3$ for Holling type III function, or any positive value of $x$ for the generalised Michaelis-Menten function.
...
<<<<<<< .mine
Set the $x$ parameter as $2$ for a Holling Type II process, as $3$ for a Holling Type III process, and as any positive real number for a generalised Michaelis-Menten process.
=======
The prey-switching predation mortality takes into account the availability of all prey groups in the model, and the electivity of the predator for each prey group. The predator abundance (or biomass) $P_{total}$ is calculated as above. Then for each prey group (which might contain a number of categories), the vulnerable abundance or biomass $V(g)_{total}$ is calculated as above. 
>>>>>>> .r4848

<<<<<<< .mine
Note that the mortality function will be applied as a biomass or an abundance depending on the value of the \subcommand{as\_biomass} switch.
=======
\TODO
>>>>>>> .r4848
...
The density-dependent prey-switching mortality function applies predation mortality from any single predator group to all its prey groups at once. It removes a number (or biomass) from each prey group according to the total (selected) abundance (or biomass) of this prey group, the total (selected) abundance (or biomass) of all other prey groups, some 'predator' abundance (or biomass), the preference (electivity) of the predator to each prey group, and constrained by a maximum exploitation rate. Each prey group can be composed of a number of individual categories, so can the predator group. 

For example, the mortality applied to user-defined prey group $g$ of category $k$, with the numbers removed at age $l$ determined by a selectivity-at-age $S(l)$ is applied as follows:
...
Then, calculate the total vulnerable abundance (or biomass) of prey group $g$ over all categories $k$ in $1 \ldots K$ and ages $l = 1 \ldots L$ that are subject to the mortality,
\begin{equation}
  V(g,k,l) = S(l) N(k,l)
\end{equation}

And hence define the total vulnerable abundance (or biomass) of each prey group $V(g)_{total}$ as,
\begin{equation}
  V(g)_{total}  = \sum\limits_K {\sum\limits_L {V(g,k,l)}} 
\end{equation}

The vulnerable abundance (or biomass) of every prey group $g$ in $1 \ldots G$ is calculated individually. Then the number to remove from each prey group $g$ depends on its electivity $E(g)$, on the abundance (or biomass) of all other prey groups $i$ in $1 \ldots G$, the electivity of the predator for each prey group $E(i)$, as well as the total consumption rate of the predator $CR$ and its abundance (or biomass) $P_{total}$,
\begin{equation}
  R(g)_{total}=P_{total} CR (V(g)_{total} E(g))/({\sum\limits_G {V(i)_{total} E(i)}})
\end{equation}
...
Note that prey switching is assumed to only happen within the prey groups specificed in the model, and the consumption rate by the predator has to be adjusted to represent the consumption rate of the prey groups present in the model. The electivities must sum to one and therefore $G -1$ electivities are specified in the model.

The mortality function will be applied as a biomass or an abundance depending on the value of the \subcommand{as\_biomass} switch.

\TODOend

=======
Then the number to remove for each prey group over all groups $g$ in $1 \ldots G$ will depend on the abundance (or biomass) $V(g)_{total}$ of all prey groups in $1 \ldots G$, and the electivity of the predator for each prey group $E(g)$, as well as the total consumption rate of the predator $CR$,
\begin{equation}
  R(g)_{total}=P_{total} CR (V(g)_{total} E(g))/({\sum\limits_G {V(i)_{total} E(i)}})
\end{equation}

For each prey group $g$, the number removed is then calculated as above.

Note that prey switching is assumed to only happen within the prey groups specificed in the model. The electivities must sum to one, and the consumption rate has to be adjusted to represent the consumption rate of the prey groups present in the model by the predator.
...
\subsection{\I{Derived layers}\label{sec:derived-layers}}
...
There are two size-weight relationship,s available in \SPM. The first is the naive no relationship, where the weight of an individual, irregardless of size is always 1. The second is the basic relationship. 
...
\begin{itemize}
  ...
  \begin{equation}
    \text{mean weight}=1
  \end{equation}
  \item{Basic:}\index{Basic size-weight relationship} The size-weight relationship where 
  \begin{equation}
...
%If a distribution is specified for the size-at-age relationship, then the mean weight at age is calculated over that distribution, using the following formula, which is exact for lognormal distributions, and a good approximation for a normal distribution (if the c.v. is not large),
%\begin{equation}
