...
Mortality for the annual rate\index{Annual mortality} is similar, except that the rate applied in each year the annual rate applies is defined as separate values of a constant morality rate $M$, a layer $L_a$, with a selectivity-at-age $S_l$ for each category considered. For example, if annual rates were applied to males and females between 1996 and 2000, five layers and values of M, and two categories and selectivities would need to be provided to the model.
...
For example, let the emigration applied to the population at cell $a$ in category $k$ and age $l$ be denoted $E(a,k,l)$, and given a value from an emigration layer $Le_a$  at $a$, a constant migration proportion $P$, and a selectivity-at-age $S_l$ at age $l$ for some user-defined categories $k$ then, 
\begin{equation}
