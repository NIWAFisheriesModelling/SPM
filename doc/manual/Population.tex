...
The population, at any point in time, is described by the \emph{state}, which comprises of the \emph{partition} and any \emph{derived quantities}. The partition\index{Partition}\index{Model ! partition} is a 4-dimensional matrix, comprising of the spatial structure\index{Spatial structure}, and within each spatial location, the population structure\index{Population structure}. 
...
The names and number of categories are user defined, but there  just be at least one category defined for a model. The ages are defined as a sequence from $age_{min}$ to $age_{max}$, and the last age may optionally be a plus group.
