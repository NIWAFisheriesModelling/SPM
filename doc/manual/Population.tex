...
The Holling mortality process\index{Holling mortality} applies the Holling Type II and Type III functions \citep{Holling1959}, but generalised using the Michaelis-Menten equation \citep{MentenMichaelis1913}. Holling Type II or Type III can be obtained by setting the $x$ parameter as either $2$ or $3$ respectively. The function removes a number or biomass from a set of categories according to their abundance (or biomass) and some 'predator' abundance (or biomass) .
...
\begin{equation}
R(k,l) = L_a (a  V(k,l)^{x-1})/(b + V(k,l)^{x-1})
\end{equation}

Set the $x$ parameter as $2$ for a Holling Type II process, and as $3$ for a Holling Type III process.
