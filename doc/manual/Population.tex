...
The growth component of \SPM\ is used to evaluate the size (for example, using the age-length relationship) and mass (for example, from the length-weight relationship) of individuals. This is used to determine length frequencies or biomass of individuals at age/category. 

In an age-based model, fish size does not feature in the partition, but size-at-age is still an element of the model. Size-at-age is based on a growth curve which specifies the mean size at a given age. There are two alternative growth curves in \SPM, 
