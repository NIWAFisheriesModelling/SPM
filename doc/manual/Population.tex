...
\paragraph{Constant rate mortality:}
...
\paragraph{Annual rate mortality:}
...
\paragraph{Density-dependent Holling mortality:}
...
Note, choose $x=2$ for Holling type II function and $x=3$ for Holling type III function, or any positive value of $x$ for the generalised Michaelis-Menten function.
...
\paragraph{Density-dependent prey-switching mortality:}

The density-dependent prey-switching process\index{Density-dependent prey-switching} applies predation mortality from any single predator group to all of its prey groups simultaneously. It removes an abundance (or biomass) from each prey group according to the total (selected) abundance (or biomass) of this prey group, the total (selected) abundance (or biomass) of all other prey groups, some 'predator' abundance (or biomass), and the preference (electivity) of the predator to each prey group, but constrained by a maximum exploitation rate. Note that each prey group can be composed of a number of individual categories, as can the predator group. The electivity of the predator for each prey species is expressed as a generalised Michaelis-Menten function.
...
The electivity for each prey group $E(g)$ is a function of the abundance (or biomass) of that prey groups $V(g)_{total}$ and parameters $a(g)$, $b(g)$, and $x(g)$ of the Michaelis-Menten function for each prey group.
\begin{equation}
E(g) = (a(g)  V(g)_{total}^{x-1})/(b(g) + V(g)_{total}^{x-1})
\end{equation}

Note, choose $x=2$ for Holling type II function and $x=3$ for Holling type III function, or any positive value of $x$ for the generalised Michaelis-Menten function. Alternatively chose $x=1$ and $b=0$ for a constant electivity of value $a$.
...
Note that prey switching occurs only between prey groups specified by the process, and the total predator consumption rate represents the consumption of the predator on those prey groups alone. Also note that the electivities must sum to one. We define $G -1$ electivities in the process, with the last electivity set equal to $1-\sum\limits_{G-1}{E(g)}$.

The density-dependent prey-switching process will be applied as a biomass or an abundance depending on the value of the \subcommand{is\_abundance} switch.
