...
\SPM\ implements two spatial structures, one derived from a grid of \emph{square} cells \textemdash the default (Figure \ref{fig:SquareSpatialStructure}) and the other derived from a grid of north-south orientated \emph{hexagons} (Figure \ref{fig:HexagonSpatialStructure}). We note that the spatial grid can be of an arbitrary size, but must be rectangular. 
...
\subsubsection{\I{Model initialisation}}
...
\begin{enumerate}
	...
	\item An adjacent cell movements\index{Adjacent cell movement}, parametrised by some function of an underlying layer \textemdash equivalent to, for example, movement processes implemented in Fish Heaven \citep{1136,1135}.
...
\subsubsection{\I{Category transitions}}
...
\subsubsection*{\I{The total preference function}}
...
\subsubsection*{\I{Preference functions}}

Preference functions in \SPM\ include \subcommand{Constant}, \subcommand{Normal}, \subcommand{DoubleNormal}, \subcommand{Logistic}, \subcommand{InverseLogistic}, \subcommand{ExponentialDecay}, and \subcommand{Threshold}. These are defined as,

\begin{enumerate}
\item The \subcommand{Constant}\ preference function\index{Constant preference function}\index{Preference functions ! Constant} has dependent variable $x$ and has no parameters, and is defined as,
\begin{equation}
f(x)=x$, where $0 \leq x \leq 1
\end{equation}

\item The \subcommand{Normal}\ preference function\index{Normal preference function}\index{Preference functions ! Normal} has dependent variable $x$ and parameters $\theta = (\mu,\sigma)$, and is defined as, 
\begin{equation}\
f(x | \mu, \sigma) = 2^{-[(x- \mu)/\sigma ]^2} 
\end{equation}
 
\item The \subcommand{DoubleNormal}\ preference function\index{Double-normal preference function}\index{Preference functions ! Double-normal} has dependent variable $x$ and parameters $\theta=(\mu,\sigma_L,\sigma_R)$, and is defined as,
\begin{equation}
...
\item The \subcommand{Logistic}\ preference function \index{Logistic preference function}\index{Preference functions ! Logistic} has dependent variable $x$ and parameters $\theta = (a_{50},a_{to95})$, and is defined as,
\begin{equation}
  f(x | a_{50}, a_{to95}) = 1 / [1+19^{(a_{50}-x)/a_{to95}}]
\end{equation}

\item The \subcommand{InverseLogistic}\ preference function\index{Inverse-Logistic preference function}\index{Preference functions ! Inverse-Logistic} has dependent variable $x$ and parameters $\theta = (a_{50},a_{to95})$, and is defined as,
\begin{equation}
  f(x | a_{50}, a_{to95}) =1- 1 / [1+19^{(a_{50}-x)/a_{to95}}]
\end{equation}

\item The \subcommand{ExponentialDecay}\ preference function\index{Exponential-decay preference function}\index{Preference functions ! Exponential-decay} has dependent variable $x$ and parameters $\theta = (\lambda)$, and is defined as,
\begin{equation}
  f(x | \lambda) =\exp(-\lambda x), \text{where $x \geq 0$ and $0$ otherwise}
\end{equation}

\item The \subcommand{Threshold}\ preference function\index{Threshold preference function}\index{Preference functions ! Threshold} has dependent variable $x$ and parameters $\theta = (N,\lambda)$, and is defined as,
\begin{equation}
...
\subsection{\I{Size-at-age}\label{sec:size-at-age}}
...
\subsection{\I{Mean weight}\label{sec:mean-weight}}
...
\subsubsection[Knife-edge]{\argument{KnifeEdge}}
...
\subsubsection[All-values]{\argument{AllValues}}
...
\subsubsection[All-values-bounded]{\argument{AllValuesBounded}}
...
The increasing ogive has non-estimable parameters $L$ and $H$. The estimable parameters are $\pi_L$, $\pi_{L+1}$ \ldots $\pi_H$ (but if these are estimated, they should always be constrained to be between 0 and 1). Note that the increasing ogive is similar to the all-values-bounded ogive, but is constrained to be non-decreasing.
...
\subsubsection[Logistic producing]{\argument{LogisticProducing}}
...
\subsubsection[Double-normal]{\argument{DoubleNormal}}
...
\subsubsection[Double-exponential]{\argument{DoubleExponential}}
