...
You need to tell SPM which of the estimable parameters are to be freed by using \command{estimate} commands (see \ref{sec:estimation-syntax}). An \command{estimate} command-block looks like,
...
If you get an implausible profile, it may be a result of not using enough iterations in the minimiser (in this case, increase \subcommand{MaxIters} and/or \subcommand{MaxEvals} and retry), or the convergence criteria may not be strong enough (try setting \subcommand{GradTol} to a smaller value).
...
\item The covariance matrix is modified so as to decrease all correlations greater than \subcommand{MaxCor} down to \subcommand{MaxCor}, and similarly to increase all correlations less than  \subcommand{MaxCor} up to \subcommand{MaxCor} (the \subcommand{MaxCor} parameter defaults to 0.8). This should help to avoid getting 'stuck' in a lower-dimensional subspace.

\item The covariance matrix is then modified either by,

\begin{itemize}
\item if \subcommand{AdjustmentMethod} is \argument{covariance}: that if the variance of the $i$th parameter is nonzero and less than \subcommand{MinDiff} times the difference between the parameters' lower and upper bound, then the variance is changed, without changing the associated correlations, to $k=$MinDiff$(upper\_bound_i-lower\_bound_i)$. This is done by setting \[
...
\item if \subcommand{AdjustmentMethod} is \argument{correlation}: that if the variance of the $i$th parameter is nonzero and less than \subcommand{MinDiff} times the difference between the parameters' lower and upper bound, then its variance is changed to $k=MinDiff(upper\_bound_i-lower\_bound_i)$. This differs from (i) above in that the effect of this option is that it also modifies the resulting correlations between the ith parameter and all other parameters.
\end{itemize}

This allows a free parameter to move in the MCMC even if its variance is very small according to the inverse Hessian. In both cases, the \subcommand{MinDiff} parameter defaults to 0.0001.

\item The \subcommand{StepSize} (a scalar factor applied to the covariance matrix to improve the acceptance probability) is chosen by the user. The default is $2.4d^{-0.5}$ where $d$ is the number of free parameters, as recommended by Gelman et al. \citep{823}, though experience has shown that this may be too high and can lead to a low acceptance rate. 
\end{itemize}
...
The variance-covariance matrix of this sub-sample of chain is calculated. As above, correlations greater than \subcommand{MaxCor} are reduced to \subcommand{MaxCor}, correlations less than \subcommand{MaxCor} are increased to  \subcommand{MaxCor}, and very small non-zero variances are increased (\subcommand{CovarianceAdjustment} and \subcommand{MinDiff}. The result is the new variance-covariance matrix of the proposal distribution.
