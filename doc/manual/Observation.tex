...
In all cases, the process error has more effect on small errors than on large ones. Be clear that a large value for the multinomial process error means a small process error.
...
All of the observation types in \SPM\ can be used as pseudo-observations. To specify a pseudo-observation, include the observation as usual in the \config, but with a likelihood of \argument{type=none}. Note that;
...
\begin{itemize}
  ...
  \item The subcommands \subcommand{likelihood}, \subcommand{obs}, \subcommand{error\_value} and \subcommand{process\_error} have no effect when generating the expected values for the pseudo-observation.   \item \SPM\ cannot simulate from pseudo-observations.
\end{itemize}
