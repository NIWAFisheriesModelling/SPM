...
Relative proportions present observations are observations of either a relative proportion of sampling efforts that detected the presence of some individuals from a set of categories after applying a selectivity. This assumes that the relative probability of detecting an individual in a cell is proportional to the abundance (in number of individuals) for that cell. 

Each observation is for a given year and time-step, for some selected age classes of the population (i.e., for a range of ages multiplied by a selectivity), for categories aggregated over a set of spatial cells. Further, you need to provide the label of the catchability coefficient $q$, which can either be estimated or fixed. The catchability coefficient must be between 0 and 1.
...
For observations $O_i$, N. $N_i$, and expected values $qE_i$, the negative log-likelihood is defined as;

For observed proportions $O_i$ for cell $i$, where $qE_i$ are the expected proportions in cell $i$, scaled by a catchability coefficient $q$ (where $0 < q le 1$), and $N_i$ is the effective sample size in cell $i$, then the negative log-likelihood is defined as;  
