    ...
    while (vPropPtr != mvProportionMatrix.end()) {
      ...
      if (bRescale) {
        ...
        if (fabs(1.0-dRunningTotal)> 0.001) {
          throw string((*vPropPtr).first + ERROR_EQUAL_PROPORTION_ONE + convertDouble(dRunningTotal));
        }
      }
...
void CProportionsAtAgeObservation::execute() {
  ...
  try {
    ...
    while (vPropPtr != mvProportionMatrix.end()) {
      ...
      dScore = -lnFactorial(dN);

      for (int i = 0; i < iArraySize; ++i) {

        // Get our Proportion
        if(!isZero(dRunningTotal))
          dCurrentProp = pAgeResults[i] / dRunningTotal;
        else
          dCurrentProp = 0.0;
        dScore += lnFactorial(dN * ((*vPropPtr).second)[i]) - dN * ((*vPropPtr).second)[i] * log(zeroFun(dCurrentProp,dR));
      }
