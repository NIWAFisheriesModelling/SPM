...
void CGaussianPreferenceFunction::validate() {
  try {
    ...
    if (dRho <= -1.0 || dRho >= 1.0)
      CError::errorLessThanEqualTo(PARAM_RHO, PARAM_BOUNDS);
...
//************************************************************************************************
//	void inv_cdf is a function that takes three parameters double quantile
...
double CGaussianPreferenceFunction::getResult(int RIndex, int CIndex, int TRIndex, int TCIndex) {
  ...
  try {
    ...
    if((vPDFTypes[0] == PARAM_NORMAL) && if(vPDFTypes[1] == PARAM_NORMAL)) {
      double x1 = dLayervalue[0];
      double x2 = dLayervalue[1];
 
    // dRet = 1/(2*PI*sqrt(1-dRho) * exp (-(normsInv(dCDF1)^2 - 2*dRho*normsInv(dCDF1)*normsInv(dCDF2) + normsInv(dCDF2)^2)/(2-2*dRho^2))* dPDF1 * dPDF2;
	// Evaluate using Gaussian copula
